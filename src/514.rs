...
impl Solution {
    ...
    fn dfs(
        ...
        let mut ret = i32::MAX;
        for &candidate_index in index.get(&key[kpos]).unwrap().iter() {
            // using current index as one option and dive in
            let diff = (candidate_index as i32 - rpos as i32).abs();
            let step = diff.min(ring.len() as i32 - diff);

            ret = ret.min(Self::dfs(ring, key, index, dp, candidate_index, kpos + 1) + step);
        }
